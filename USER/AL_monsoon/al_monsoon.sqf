...
params ["_direction_monsoon","_duration_monsoon","_effect_on_objects","_debris_branches","_rain_fog","_rain_drop","_thunder_steroids"];

delay_thunder = _this select 7; publicVariable "delay_thunder";
al_monsoon_om = true; publicVariable "al_monsoon_om";
...
execVM "USER\AL_monsoon\alias_hunt.sqf";
...
[_duration_monsoon] spawn {
	...
	al_monsoon_om = false; publicVariable "al_monsoon_om";
...
[[_debris_branches,_rain_fog],"USER\AL_monsoon\alias_monsoon_effect.sqf"] remoteExec ["execVM",0,true];
if(_rain_drop) then {["USER\AL_monsoon\rain_drop.sqf"] remoteExec ["execVM",0,true]};
if(_thunder_steroids) then 
{
	_rand_pl = [] execVM "USER\AL_monsoon\alias_hunt.sqf";
	...
	[] spawn 
	{
		...
		while {al_monsoon_om} do 
		{
			...
			//poz_f_1 = getrelpos [hunt_alias,700+random 1300, random 360] call BIS_fnc_relPos;
			poz_f_1 = hunt_alias getRelPos [700+random 1300, 360]; publicVariable "poz_f_1";
			...
			[[poz_f_1,_tip_tunet,_numar_sclipiri,_sound_tunet,_storm_clouds],"USER\AL_monsoon\thunder_storm.sqf"] remoteExec ["execVM"];
			sleep _delayth2;
		};
	};
};
	...
	while {al_monsoon_om} do 
	{
		...
		if (count ar_obj_eligibl > 0) then 
		{
			...
			[] spawn {
				...
				["USER\AL_monsoon\camera_shake.sqf"] remoteExec ["execVM"];
			};
			_xblow	= 0.1+random 5;
			_yblow	= 0.1+random 5;
